...
struct mtmd_context {
    ...
    std::unordered_map<std::string, mmproj_kv_cache_entry> mmproj_kv_cache;
    std::mutex cache_mutex;
    size_t max_cache_size_bytes;
    size_t current_cache_size_bytes;
    size_t max_ram_entries;  // max entries to keep in RAM (default: 10)
    std::string disk_cache_dir;  // directory for disk cache
    ...
    mtmd_context(const char * mmproj_fname,
        ...
        max_ram_entries(10)  // keep 10 entries in RAM by default
    {
...
// mmproj KV cache management - two-tier: RAM (hot) + Disk (cold)

// Helper to get disk cache file path
static std::string mtmd_kv_cache_get_disk_path(mtmd_context * ctx, const std::string & image_id) {
    if (ctx->disk_cache_dir.empty()) {
        return "";
    }
    return ctx->disk_cache_dir + "/mmproj_cache_" + image_id + ".bin";
}

// Save entry to disk
static bool mtmd_kv_cache_save_to_disk(mtmd_context * ctx, const mtmd_context::mmproj_kv_cache_entry & entry) {
    std::string path = mtmd_kv_cache_get_disk_path(ctx, entry.image_id);
    if (path.empty()) {
        return false;
    }
    ...
    // Write header
    uint32_t magic = 0x4D4D4B56; // "MMKV"
    uint32_t version = 1;
    fwrite(&magic, sizeof(magic), 1, fp);
    fwrite(&version, sizeof(version), 1, fp);

    // Write metadata
    uint32_t id_len = entry.image_id.size();
    fwrite(&id_len, sizeof(id_len), 1, fp);
    fwrite(entry.image_id.c_str(), 1, id_len, fp);
    fwrite(&entry.pos_start, sizeof(entry.pos_start), 1, fp);
    fwrite(&entry.pos_end, sizeof(entry.pos_end), 1, fp);
    fwrite(&entry.n_tokens, sizeof(entry.n_tokens), 1, fp);

    // Write data size and data
    uint64_t data_size = entry.kv_cache_data.size();
    fwrite(&data_size, sizeof(data_size), 1, fp);
    if (data_size > 0) {
        fwrite(entry.kv_cache_data.data(), 1, data_size, fp);
    }
...
static bool mtmd_kv_cache_load_from_disk(mtmd_context * ctx, const std::string & image_id,
                                          mtmd_context::mmproj_kv_cache_entry & entry) {
    std::string path = mtmd_kv_cache_get_disk_path(ctx, image_id);
    if (path.empty()) {
        return false;
    }
    ...
    if (fread(&magic, sizeof(magic), 1, fp) != 1 || magic != 0x4D4D4B56) {
        fclose(fp);
        return false;
    }
    ...
    // Read metadata
    uint32_t id_len;
    if (fread(&id_len, sizeof(id_len), 1, fp) != 1) {
        fclose(fp);
        return false;
    }
    std::string id(id_len, '\0');
    if (fread(&id[0], 1, id_len, fp) != id_len) {
        fclose(fp);
        return false;
    }

    entry.image_id = id;
    if (fread(&entry.pos_start, sizeof(entry.pos_start), 1, fp) != 1) {
        fclose(fp);
        return false;
    }
    if (fread(&entry.pos_end, sizeof(entry.pos_end), 1, fp) != 1) {
        fclose(fp);
        return false;
    }
    if (fread(&entry.n_tokens, sizeof(entry.n_tokens), 1, fp) != 1) {
        fclose(fp);
        return false;
    }
    ...
    entry.kv_cache_data.resize(data_size);
    if (data_size > 0) {
        if (fread(entry.kv_cache_data.data(), 1, data_size, fp) != data_size) {
            fclose(fp);
            return false;
        }
    }

    entry.timestamp = ggml_time_ms();
    fclose(fp);
    return true;
}

// Evict oldest entries to disk when RAM is full
static void mtmd_kv_cache_evict_to_disk(mtmd_context * ctx) {
    if (ctx->mmproj_kv_cache.size() <= ctx->max_ram_entries) {
        return;
    }

    LOG_INF("mmproj KV cache: RAM full (%zu entries), evicting to disk...\n", ctx->mmproj_kv_cache.size());

    // Sort by timestamp (oldest first)
    std::vector<std::pair<uint64_t, std::string>> entries;
    for (const auto& [id, entry] : ctx->mmproj_kv_cache) {
        entries.push_back({entry.timestamp, id});
    }
    std::sort(entries.begin(), entries.end());

    // Evict oldest entries to disk until we're at max_ram_entries / 2
    size_t target_count = ctx->max_ram_entries / 2;
    size_t evicted = 0;

    for (const auto& [timestamp, id] : entries) {
        if (ctx->mmproj_kv_cache.size() <= target_count) {
            break;
        }

        auto it = ctx->mmproj_kv_cache.find(id);
        if (it != ctx->mmproj_kv_cache.end()) {
            // Try to save to disk
            if (!ctx->disk_cache_dir.empty()) {
                if (mtmd_kv_cache_save_to_disk(ctx, it->second)) {
                    LOG_INF("mmproj KV cache: saved %s to disk\n", id.c_str());
                } else {
                    LOG_WRN("mmproj KV cache: failed to save %s to disk, deleting\n", id.c_str());
                }
            }

            ctx->current_cache_size_bytes -= it->second.kv_cache_data.size();
            ctx->mmproj_kv_cache.erase(it);
            evicted++;
        }
    }

    LOG_INF("mmproj KV cache: evicted %zu entries to disk, RAM now has %zu entries\n",
            evicted, ctx->mmproj_kv_cache.size());
}

bool mtmd_kv_cache_save(mtmd_context * ctx,
    ...
    if (ctx->max_cache_size_bytes == 0) {
        return false;
    }

    // Check if already in cache
    if (ctx->mmproj_kv_cache.find(image_id) != ctx->mmproj_kv_cache.end()) {
        // Already cached, just update timestamp
        ctx->mmproj_kv_cache[image_id].timestamp = ggml_time_ms();
        return true;
    }

    // Get size for partial state (recurrent cache only)
    size_t state_size = llama_state_seq_get_size(lctx, 0, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
    if (state_size == 0) {
        return false;
    }

    std::vector<uint8_t> kv_data(state_size);
    size_t written = llama_state_seq_get_data(lctx, kv_data.data(), state_size, 0, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
    if (written == 0) {
        return false;
    }
    ...
    entry.kv_cache_data = std::move(kv_data);
    entry.pos_start = pos_start;
    entry.pos_end = pos_start + n_tokens;
    entry.n_tokens = n_tokens;
    entry.timestamp = ggml_time_ms();

    ctx->current_cache_size_bytes += entry.kv_cache_data.size();
    ctx->mmproj_kv_cache[image_id] = std::move(entry);

    LOG_INF("mmproj KV cache: saved %s to RAM (%zu MB), total RAM entries: %zu\n",
            image_id, state_size / (1024*1024), ctx->mmproj_kv_cache.size());

    // Evict to disk if RAM is full
    mtmd_kv_cache_evict_to_disk(ctx);

    return true;
}

bool mtmd_kv_cache_restore(mtmd_context * ctx,
                           struct llama_context * lctx,
                           const char * image_id,
    ...
    auto it = ctx->mmproj_kv_cache.find(image_id);
    if (it != ctx->mmproj_kv_cache.end()) {
        const auto& entry = it->second;
        if (entry.kv_cache_data.empty()) {
            return false;
        }

        size_t read = llama_state_seq_set_data(lctx, entry.kv_cache_data.data(),
                                               entry.kv_cache_data.size(), 0,
                                               LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
        if (read == 0) {
            return false;
        }

        it->second.timestamp = ggml_time_ms();
        LOG_INF("mmproj KV cache: restored %s from RAM\n", image_id);
        return true;
    }

    // Not in RAM, try to load from disk
    mtmd_context::mmproj_kv_cache_entry entry;
    if (mtmd_kv_cache_load_from_disk(ctx, image_id, entry)) {
        // Restore from disk
        size_t read = llama_state_seq_set_data(lctx, entry.kv_cache_data.data(),
                                               entry.kv_cache_data.size(), 0,
                                               LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
        ...
        // Move to RAM cache (will trigger eviction if needed)
        ctx->current_cache_size_bytes += entry.kv_cache_data.size();
        ctx->mmproj_kv_cache[image_id] = std::move(entry);

        LOG_INF("mmproj KV cache: restored %s from disk to RAM\n", image_id);

        // Evict to disk if RAM is now full
        mtmd_kv_cache_evict_to_disk(ctx);
...
void mtmd_kv_cache_clear(mtmd_context * ctx) {
    std::lock_guard<std::mutex> lock(ctx->cache_mutex);
    ctx->mmproj_kv_cache.clear();
    ctx->current_cache_size_bytes = 0;
}

void mtmd_kv_cache_set_max_size(mtmd_context * ctx, size_t max_size_bytes) {
    std::lock_guard<std::mutex> lock(ctx->cache_mutex);
    ctx->max_cache_size_bytes = max_size_bytes;
}

void mtmd_kv_cache_set_max_entries(mtmd_context * ctx, size_t max_entries) {
    std::lock_guard<std::mutex> lock(ctx->cache_mutex);
    ctx->max_ram_entries = max_entries;
}

void mtmd_kv_cache_set_disk_dir(mtmd_context * ctx, const char * dir) {
    std::lock_guard<std::mutex> lock(ctx->cache_mutex);
    ctx->disk_cache_dir = dir ? dir : "";
}
