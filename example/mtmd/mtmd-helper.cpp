...
int32_t mtmd_helper_eval_chunk_single(mtmd_context * ctx,
    ...
    if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        ...
        const char * image_id = mtmd_input_chunk_get_id(chunk);
        if (image_id && strlen(image_id) > 0) {
            int32_t n_tokens = mtmd_input_chunk_get_n_tokens(chunk);
            if (mtmd_kv_cache_save(ctx, lctx, image_id, pos_start, n_tokens)) {
                LOG_INF("saved KV cache for %s: id=%s, pos=%d, n_tokens=%d\n", name, image_id, pos_start, n_tokens);
            }
        }
    } else {
