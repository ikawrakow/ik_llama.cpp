...
// mmproj KV cache management for recurrent models
// save KV cache state for an image after decoding
MTMD_API bool mtmd_kv_cache_save(mtmd_context * ctx,
                                  struct llama_context * lctx,
                                  const char * image_id,
                                  llama_pos pos_start,
                                  int32_t n_tokens);

// restore KV cache state for an image
MTMD_API bool mtmd_kv_cache_restore(mtmd_context * ctx,
                                     struct llama_context * lctx,
                                     const char * image_id,
                                     llama_pos pos_target);

// clear all cached KV cache
MTMD_API void mtmd_kv_cache_clear(mtmd_context * ctx);

// set max cache size in bytes (RAM)
MTMD_API void mtmd_kv_cache_set_max_size(mtmd_context * ctx, size_t max_size_bytes);

// set max RAM cache entries (default: 10)
MTMD_API void mtmd_kv_cache_set_max_entries(mtmd_context * ctx, size_t max_entries);

// set disk cache directory for overflow entries
MTMD_API void mtmd_kv_cache_set_disk_dir(mtmd_context * ctx, const char * dir);
