...
bool server_context::load_model(const gpt_params& params_) {
    ...
    if (!mmproj_path.empty()) {
        ...
        if (params_base.mmproj_kv_cache_enabled) {
            mtmd_kv_cache_set_max_size(mctx, params_base.mmproj_kv_cache_max_mb * 1024 * 1024);
            mtmd_kv_cache_set_max_entries(mctx, 10);  // keep 10 in RAM
            // Use system temp dir or current dir for disk cache
            std::string disk_dir = "/tmp/llama_mmproj_cache";
            mtmd_kv_cache_set_disk_dir(mctx, disk_dir.c_str());
            LOG_INFO("mmproj KV cache enabled", { 
                {"max_size_mb", params_base.mmproj_kv_cache_max_mb},
                {"ram_entries", 10},
                {"disk_dir", disk_dir}
            });
        } else {
            mtmd_kv_cache_set_max_size(mctx, 0);
            LOG_INFO("mmproj KV cache disabled\n", {});
        }
        ...
        for (auto& slot : slots) {
            ...
            if (slot.state == SLOT_STATE_IDLE && slot.command == SLOT_COMMAND_LOAD_PROMPT) {
                ...
                if (slot.n_past_prompt < slot.n_prompt_tokens
                    ...
                    // try to restore KV cache first
                    const auto& chunk = slot.prompt_tokens.find_chunk(slot.n_past_prompt);
                    const char * image_id = mtmd_input_chunk_get_id(chunk.get());
                    
                    bool cache_hit = false;
                    if (image_id && strlen(image_id) > 0 && mctx) {
                        llama_pos p1 = slot.cache_tokens.pos_next() + slot.n_past_prompt - slot.n_past;
                        if (mtmd_kv_cache_restore(mctx, ctx, image_id, p1)) {
                            int32_t n_tokens = mtmd_input_chunk_get_n_tokens(chunk.get());
                            slot.n_past += n_tokens;
                            slot.n_past_prompt += n_tokens;
                            slot.n_prompt_tokens_processed += n_tokens;
                            slot.cache_tokens.push_back(chunk.get());
                            slot.image_just_processed = true;
                            cache_hit = true;
                            LOG_INFO("restored KV cache for image", {
                                { "id_slot", slot.id },
                                { "image_id", image_id },
                                { "n_tokens", n_tokens }
                            });
