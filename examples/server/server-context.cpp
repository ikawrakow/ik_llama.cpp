...
bool server_perplexity_state::save(const std::string & filename) const {
    std::ofstream file(filename, std::ios::binary);
    if (!file) return false;

    const uint32_t magic   = 0x504C5459;
    const uint32_t version = 2;
    file.write((const char *)&magic,   sizeof(magic));
    file.write((const char *)&version, sizeof(version));

    file.write((const char *)&n_ctx,       sizeof(n_ctx));
    file.write((const char *)&n_batch,     sizeof(n_batch));
    file.write((const char *)&ppl_stride,  sizeof(ppl_stride));
    file.write((const char *)&nll,         sizeof(nll));
    file.write((const char *)&nll2,        sizeof(nll2));
    file.write((const char *)&count,       sizeof(count));
    file.write((const char *)&chunk_index, sizeof(chunk_index));

    size_t n_tokens = tokens.size();
    file.write((const char *)&n_tokens, sizeof(n_tokens));
    if (n_tokens) file.write((const char *)tokens.data(), n_tokens * sizeof(llama_token));

    size_t n_all = all_tokens.size();
    file.write((const char *)&n_all, sizeof(n_all));
    if (n_all) file.write((const char *)all_tokens.data(), n_all * sizeof(llama_token));

    return file.good();
}

bool server_perplexity_state::load(const std::string & filename) {
    ...
    if (magic != 0x504C5459 || version < 1 || version > 2) return false;

    file.read((char *)&n_ctx,      sizeof(n_ctx));
    file.read((char *)&n_batch,    sizeof(n_batch));
    file.read((char *)&ppl_stride, sizeof(ppl_stride));
    file.read((char *)&nll,        sizeof(nll));
    file.read((char *)&nll2,       sizeof(nll2));
    file.read((char *)&count,      sizeof(count));

    if (version >= 2) {
        file.read((char *)&chunk_index, sizeof(chunk_index));
    } else {
        chunk_index = 0;
    }

    size_t n_tokens;
    file.read((char *)&n_tokens, sizeof(n_tokens));
    tokens.resize(n_tokens);
    if (n_tokens) file.read((char *)tokens.data(), n_tokens * sizeof(llama_token));

    size_t n_all;
    file.read((char *)&n_all, sizeof(n_all));
    all_tokens.resize(n_all);
    if (n_all) file.read((char *)all_tokens.data(), n_all * sizeof(llama_token));
...
void server_context::process_perplexity_chunk(const std::vector<llama_token> & new_tokens,
                                              server_task_result_perplexity  & result) {
    if (!perplexity_state) {
        perplexity_state = std::make_unique<server_perplexity_state>();
        perplexity_state->n_ctx   = params_base.n_ctx;
        perplexity_state->n_batch = params_base.n_batch;
    }

    auto & state = *perplexity_state;
    state.all_tokens.insert(state.all_tokens.end(), new_tokens.begin(), new_tokens.end());
    state.tokens.insert(state.tokens.end(), new_tokens.begin(), new_tokens.end());

    const int n_ctx   = state.n_ctx;
    const int n_batch = state.n_batch;
    const int n_vocab = llama_n_vocab(model);
    const int first   = n_ctx / 2;
    const bool add_bos = llama_should_add_bos_token(model);

    LLAMA_LOG_DEBUG("[PPL DEBUG] process_perplexity_chunk: n_ctx=%d n_batch=%d n_vocab=%d first=%d buffered=%zu\n",
            n_ctx, n_batch, n_vocab, first, state.tokens.size());

    llama_batch batch = llama_batch_init(std::min(n_batch, n_ctx), 0, 1);

    while ((int)state.tokens.size() >= n_ctx) {
        std::vector<llama_token> chunk(state.tokens.begin(), state.tokens.begin() + n_ctx);
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            batch.n_tokens = 0;
            const int seq_start = batch_start;
            const llama_token token_org = chunk[seq_start];
            if (add_bos && j == 0) {
                chunk[seq_start] = llama_token_bos(model);
            }

            for (int k = 0; k < batch_size; ++k) {
                const int idx = k;
                batch.token[idx]    = chunk[seq_start + k];
            ...
            chunk[seq_start] = token_org;

            LLAMA_LOG_DEBUG("[PPL DEBUG] batch %d/%d n_tokens=%d pos=[%d..%d] first_token=%d n_outputs=%d\n",
                    j + 1, num_batches, batch_size,
                    batch.pos[0], batch.pos[batch_size - 1],
                    batch.token[0], n_outputs);

            int decode_ret = llama_decode(ctx, batch);

            if (decode_ret != 0) {
                decode_failed = true;
                LLAMA_LOG_DEBUG("[PPL DEBUG] decode FAILED on batch %d/%d with ret=%d\n", j + 1, num_batches, decode_ret);
                break;
            }
        ...
        // Reference uses llama_get_logits_ith(ctx, first) directly when num_batches == 1
        const float * all_logits = num_batches > 1
            ? logits.data()
            : llama_get_logits_ith(ctx, first);

        if (!all_logits) {
            LLAMA_LOG_DEBUG("[PPL DEBUG] failed to get logits\n");
            result.error = true;
            llama_batch_free(batch);
            return;
        }

        // Compute log-probabilities for tokens [first .. n_ctx-2]
        double chunk_nll  = 0.0;
        double chunk_nll2 = 0.0;
        for (int j = first; j < n_ctx - 1; ++j) {
            const float * tok_logits = all_logits + (size_t)(j - first) * n_vocab;

            // ---- identical to perplexity.cpp::log_softmax() ----
            float max_logit = tok_logits[0];
            for (int k = 1; k < n_vocab; ++k) {
                max_logit = std::max(max_logit, tok_logits[k]);
            }

            double sum_exp = 0.0;
            for (int k = 0; k < n_vocab; ++k) {
                sum_exp += expf(tok_logits[k] - max_logit);
            }

            // keep full double precision, do NOT cast to float
            double log_softmax = tok_logits[chunk[j + 1]] - max_logit - log(sum_exp);
            double v = -log_softmax;
            // --------------------------------------------------

            if (j < first + 3) {
                LLAMA_LOG_DEBUG("[PPL DEBUG] j=%d next_token=%d max_logit=%f log_softmax=%.10f v=%.10f\n",
                        j, chunk[j+1], max_logit, log_softmax, v);
            }
        ...
        state.nll   += chunk_nll;
        state.nll2  += chunk_nll2;
        state.count += n_ctx - 1 - first;
        state.chunk_index++;

        LLAMA_LOG_DEBUG("[PPL DEBUG] chunk_nll=%f chunk_nll2=%f count=%ld perplexity_so_far=%f\n",
                chunk_nll, chunk_nll2, state.count, state.count > 0 ? std::exp(state.nll / state.count) : 0.0);

        state.tokens.erase(state.tokens.begin(), state.tokens.begin() + n_ctx);
    }

    llama_batch_free(batch);

    result.nll              = state.nll;
    result.nll2             = state.nll2;
    result.count            = state.count;
    result.perplexity       = state.count > 0 ? std::exp(state.nll / state.count) : 0.0;
    result.tokens_processed = state.count;
    result.tokens_total     = state.all_tokens.size();
    result.chunk_index      = state.chunk_index;
    result.error            = false;
}

bool server_context::perplexity_save(const std::string & filename) {
    return perplexity_state ? perplexity_state->save(filename) : false;
}

bool server_context::perplexity_load(const std::string & filename) {
    if (!perplexity_state) {
        perplexity_state = std::make_unique<server_perplexity_state>();
    }
    return perplexity_state->load(filename);
}

void server_context::perplexity_reset() {
    perplexity_state.reset();
}
...
void server_context::process_single_task(server_task&& task) {
    ...
    case SERVER_TASK_TYPE_PERPLEXITY:
    {
        // perplexity needs exclusive access to the model context
        if (!slots_idle()) {
            queue_tasks.defer(std::move(task));
            break;
        }
        ...
        if (task.data.contains("tokens")) {
            new_tokens = task.data["tokens"].get<std::vector<llama_token>>();
        } else if (task.data.contains("text")) {
            new_tokens = ::common_tokenize(ctx, task.data["text"].get<std::string>(), false);
        } else {
            send_error(task, "perplexity requires 'text' or 'tokens'", ERROR_TYPE_INVALID_REQUEST);
            break;
        }

        if (!perplexity_state) {
            perplexity_state = std::make_unique<server_perplexity_state>();
            perplexity_state->n_ctx   = json_value(task.data, "n_ctx",   params_base.n_ctx);
            perplexity_state->n_batch = json_value(task.data, "n_batch", params_base.n_batch);
        }

        LLAMA_LOG_DEBUG("[PPL DEBUG] task: requested n_ctx=%d n_batch=%d actual_llama_n_ctx=%d n_new_tokens=%zu\n",
                perplexity_state->n_ctx, perplexity_state->n_batch,
                llama_n_ctx(ctx), new_tokens.size());

        server_task_result_perplexity ppl_res;
        ppl_res.id   = task.id;
        ppl_res.stop = true;
        try {
            process_perplexity_chunk(new_tokens, ppl_res);
        } catch (const std::exception & e) {
            LLAMA_LOG_DEBUG("[PPL DEBUG] EXCEPTION in process_perplexity_chunk: %s\n", e.what());
            ppl_res.error = true;
        }
        ...
        if (ppl_res.error) {
            res.error = true;
            res.data  = format_error_response("perplexity evaluation failed", ERROR_TYPE_SERVER);
        } else {
            try {
                res.error = false;
                res.data  = ppl_res.to_json();
                LLAMA_LOG_DEBUG("[PPL DEBUG] result json prepared ok\n");
            } catch (const std::exception & e) {
                LLAMA_LOG_DEBUG("[PPL DEBUG] EXCEPTION in to_json/dump: %s\n", e.what());
                res.error = true;
                res.data  = format_error_response("perplexity json serialization failed", ERROR_TYPE_SERVER);
            }
        }
        queue_results.send(res);
    } break;

    case SERVER_TASK_TYPE_PERPLEXITY_STATE:
    {
        server_task_result_perplexity ppl_res;
        ppl_res.id   = task.id;
        ppl_res.stop = true;
        if (perplexity_state) {
            ppl_res.nll              = perplexity_state->nll;
            ppl_res.nll2             = perplexity_state->nll2;
            ppl_res.count            = perplexity_state->count;
            ppl_res.perplexity       = perplexity_state->count > 0
                                       ? std::exp(perplexity_state->nll / perplexity_state->count)
                                       : 0.0;
            ppl_res.tokens_processed = perplexity_state->count;
            ppl_res.tokens_total     = perplexity_state->all_tokens.size();
            ppl_res.chunk_index      = perplexity_state->chunk_index;
        }
    ...
    case SERVER_TASK_TYPE_PERPLEXITY_SAVE:
    {
        std::string filename = json_value(task.data, "filename", std::string("perplexity_state.bin"));
        bool ok = perplexity_save(filename);

        server_task_result res;
        res.id    = task.id;
        res.stop  = true;
        res.error = !ok;
        res.data  = { {"saved", ok}, {"filename", filename} };
        queue_results.send(res);
    } break;

    case SERVER_TASK_TYPE_PERPLEXITY_LOAD:
    {
        std::string filename = json_value(task.data, "filename", std::string("perplexity_state.bin"));
        bool ok = perplexity_load(filename);

        server_task_result res;
        res.id    = task.id;
        res.stop  = true;
        res.error = !ok;
        res.data  = { {"loaded", ok}, {"filename", filename} };
        queue_results.send(res);
    } break;

    case SERVER_TASK_TYPE_PERPLEXITY_RESET:
    {
        perplexity_reset();
