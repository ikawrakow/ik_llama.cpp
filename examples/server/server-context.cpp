...
void server_context::send_token_results(completion_token_outputs& results, server_slot& slot, int32_t n) {
    ...
    for (auto& it : results) {
        ...
        count++;
        if (!has_next) {
            // If stopped by limit, continue processing the buffer to ensure all generated tokens are sent
    ...
    // If we finished the loop due to limit (and didn't release yet), release now
...
inline int32_t check_ban_phrase(const server_slot& slot) {
    ...
    for (auto it : slot.ban_phrases) {
        size_t start = string_buffer_lower.find(it);
    ...
    for (const auto& pattern : slot.ban_regex) {
        try {
            ...
            // Should be caught during validation, but safe fallback
            continue;
        }
    }
    ...
    if (found) {
        ...
        LLAMA_LOG_DEBUG("Banned string/regex dectected: %s\n ", string_buffer.substr(best_start).c_str());
        n = find_n_tokens_from_string(slot.ctx, tokens, best_start, 0, unused);
...
inline void rewind_context(server_slot& slot, int32_t n_rewind) {
    ...
    // Adjust decoded count so user doesn't lose budget on banned tokens
