...
I personally don't take `IQ1_S` and `IQ1_M` very seriously, so did not implement those. The only reason `IQ1_S` is implemented is that there was a user asking for `IQ1_S`, so I added it in PR #212. It then turned out this specific user was only asking for it to copy the code into KTransformers.
