...
struct ggml_tensor * llm_build_context::build_glm4_moe_layer(
    ...
    // Attention
    ggml_tensor * cur = llm_build_norm(ctx0, inpL, hparams, layer.attn_norm, NULL, LLM_NORM_RMS, cb, il);
    cb(cur, "attn_norm", il);

    {
        ...
        const int64_t n_embd_head_k = hparams.n_embd_head_k(0);
        Qcur = ggml_reshape_3d(ctx0, Qcur, n_embd_head_k, Qcur->ne[0]/n_embd_head_k, n_layer_tokens);
        if (layer.attn_q_norm) {
            Qcur = llm_build_norm(ctx0, Qcur, hparams, layer.attn_q_norm, NULL, LLM_NORM_RMS, cb, il);
        }
        Kcur = ggml_reshape_3d(ctx0, Kcur, n_embd_head_k, Kcur->ne[0]/n_embd_head_k, n_layer_tokens);
        if (layer.attn_k_norm) {
            Kcur = llm_build_norm(ctx0, Kcur, hparams, layer.attn_k_norm, NULL, LLM_NORM_RMS, cb, il);
        }
        ...
        cur = llm_build_kv(ctx0, lctx, kv_self, gf,
                layer.wo, NULL,
                Kcur, Vcur, Qcur, kq_mask,
                n_layer_tokens, kv_head_write, n_kv,
                1.0f/sqrtf(float(n_embd_head)), cb, il);
    }
    ...
    if ((uint32_t) il < hparams.n_layer_dense_lead) {
        cur = llm_build_ffn(ctx0, lctx, layer.ffn_norm, ffn_inp,
        ...
        cur = llm_build_std_moe_ffn(ctx0, lctx, layer.ffn_norm, ffn_inp,
                layer.ffn_gate_inp,  NULL,
                layer.ffn_up_exps,   NULL,
                layer.ffn_gate_exps, NULL,
                layer.ffn_down_exps, NULL,
                ...
                LLM_FFN_SILU, hparams.expert_weights_norm, true, hparams.expert_weights_scale,
                (llm_expert_gating_func_type) hparams.expert_gating_func,
                LLM_FFN_SILU, cb, il, gf, true, layer.ffn_up_gate_exps);
    }
...
ggml_cgraph * llm_build_context::build_glm4_moe_mtp() {
    struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, llama_model_max_nodes(model, n_tokens), false);
    ...
    const int n_iters = (n_draft <= 0) ? 1 : n_draft;

    struct ggml_tensor * inp_pos = build_inp_pos();

    // Hidden states from main model
    ...
    ggml_tensor * mtp_embd = mtp_layer.nextn.embed_tokens ? mtp_layer.nextn.embed_tokens : model.tok_embd;
    ...
    ggml_tensor * warmup_KQ_mask = nullptr;
    ggml_tensor * warmup_inp_out_ids = nullptr;

    if (n_draft <= 0) {
        warmup_KQ_mask = build_inp_KQ_mask();
        warmup_inp_out_ids = build_inp_out_ids();
        lctx.inp_KQ_mask_draft.clear();
    } else {
        lctx.inp_KQ_mask_draft.resize(n_iters);
        for (int iter = 0; iter < n_iters; iter++) {
            const int32_t iter_n = (iter == 0 && n_accepted > 0) ? (n_accepted + 1) : 1;
            const int32_t iter_pad = GGML_PAD(iter_n, GGML_KQ_MASK_PAD);
            lctx.inp_KQ_mask_draft[iter] = ggml_new_tensor_2d(ctx0,
                flash_attn ? GGML_TYPE_F16 : GGML_TYPE_F32, n_kv, iter_pad);
    ...
    for (int iter = 0; iter < n_iters; iter++) {
        int32_t n_iter_tokens;
        ggml_tensor * iter_emb;
        ggml_tensor * iter_hidden;
        ggml_tensor * iter_pos;
        ggml_tensor * iter_mask;
        int32_t iter_kv_head;

        if (iter == 0) {
            if (n_draft <= 0) {
                // Warmup: full prompt batch
                n_iter_tokens = n_tokens;
                iter_emb      = ggml_get_rows(ctx0, mtp_embd, lctx.inp_tokens);
                iter_hidden   = hidden_states;
                iter_pos      = inp_pos;
                iter_mask     = warmup_KQ_mask;
                iter_kv_head  = kv_head;
            } else if (n_accepted > 0) {
                // Hybrid: accepted tokens + 1st draft token batched together
                n_iter_tokens = n_accepted + 1;
                ggml_tensor * ids = ggml_view_1d(ctx0, lctx.inp_tokens, n_iter_tokens, 0);
                iter_emb      = ggml_get_rows(ctx0, mtp_embd, ids);
                iter_hidden   = ggml_view_2d(ctx0, hidden_states,
                                    hparams.n_embd, n_iter_tokens, hidden_states->nb[1], 0);
                iter_pos      = ggml_view_1d(ctx0, inp_pos, n_iter_tokens, 0);
                iter_mask     = lctx.inp_KQ_mask_draft[0];
                iter_kv_head  = kv_head;
            } else {
                // Draft only: single token
                n_iter_tokens = 1;
                ggml_tensor * id = ggml_view_1d(ctx0, lctx.inp_tokens, 1, 0);
                iter_emb      = ggml_get_rows(ctx0, mtp_embd, id);
                iter_hidden   = ggml_view_1d(ctx0, hidden_states, hparams.n_embd, 0);
                iter_pos      = ggml_view_1d(ctx0, inp_pos, 1, 0);
                iter_mask     = lctx.inp_KQ_mask_draft[0];
                iter_kv_head  = kv_head;
            }
        } else {
            // Unrolled: single token from previous iteration's argmax
            n_iter_tokens = 1;
            iter_emb      = token_emb;
            iter_hidden   = ggml_view_1d(ctx0, hidden_states, hparams.n_embd,
                                (int64_t)std::max(0, n_accepted - 1) * hparams.n_embd * ggml_element_size(hidden_states));
            iter_pos      = ggml_view_1d(ctx0, inp_pos, 1,
                                (int64_t)(n_accepted + iter) * ggml_element_size(inp_pos));
            iter_mask     = lctx.inp_KQ_mask_draft[iter];
            iter_kv_head  = kv_head + n_accepted + iter;
        }

        ggml_tensor * emb_norm = llm_build_norm(ctx0, iter_emb, hparams,
            mtp_layer.nextn.enorm, NULL, LLM_NORM_RMS, cb, il);
        ggml_tensor * hid_norm = llm_build_norm(ctx0, iter_hidden, hparams,
        ...
        ggml_tensor * layer_input = llm_build_lora_mm(lctx, ctx0, mtp_layer.nextn.eh_proj, combined);

        ggml_tensor * cur = build_glm4_moe_layer(mtp_layer, layer_input,
            iter_pos, iter_mask, n_embd_head, n_iter_tokens, iter_kv_head, il, gf);
        ...
        // Extract logits
        ggml_tensor * logit_input = cur;
        if (n_draft <= 0) {
            // Warmup: crop to output tokens
            cb(cur, "result_norm", -1);
            if (warmup_inp_out_ids) {
                logit_input = ggml_get_rows(ctx0, cur, warmup_inp_out_ids);
            }
        } else if (n_iter_tokens > 1) {
            // Batched draft: take only the last token's output
            logit_input = ggml_view_1d(ctx0, cur, hparams.n_embd,
                (int64_t)(n_iter_tokens - 1) * hparams.n_embd * ggml_element_size(cur));
        }

        ggml_tensor * logits = llm_build_lora_mm(lctx, ctx0, mtp_head, logit_input);
        ...
        if (iter < n_iters - 1) {
            ggml_tensor * next_id = ggml_argmax(ctx0, logits);
            token_emb = ggml_get_rows(ctx0, mtp_embd, next_id);
        }
    }
