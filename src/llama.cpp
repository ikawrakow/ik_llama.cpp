...
void llama_sample_adaptive_p(llama_context * ctx,
                   llama_token_data_array  * candidates,
              llama_sampling_adap_context  * adap_ctx) {
    llama_sample_adaptive_p_impl(&ctx->sampling, candidates, adap_ctx);
}

void llama_prep_adaptive_p(struct llama_context * ctx, float * logits, struct llama_sampling_adap_context * adap_ctx) {
    llama_prep_adaptive_p_impl(&ctx->sampling, logits, adap_ctx);
}
...
llama_token llama_sample_token_adaptive_p(
 ...
 struct llama_sampling_adap_context * adap_ctx) {
    return llama_sample_token_adaptive_p_impl(&ctx->sampling, candidates, adap_ctx);
}
...
struct llama_sampling_adap_context * llama_init_adaptive_p(int n_vocab, const float target, const float decay, const bool updt_w_cur, const uint32_t seed) {
...
void llama_review_adaptive_p(struct llama_sampling_adap_context * adap_ctx, const int32_t n_unsent, const bool rewind_status) {
    llama_review_adaptive_p_impl(adap_ctx, n_unsent, rewind_status);
}
