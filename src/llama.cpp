    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight") || (!qs.has_output && name == tn(LLM_TENSOR_TOKEN_EMBD, "weight"))) {
        ...
        if (qs.params->token_embedding_type < GGML_TYPE_COUNT) {
            ...
            if (ftype == LLAMA_FTYPE_MOSTLY_IQ2_XXS || ftype == LLAMA_FTYPE_MOSTLY_IQ2_XS ||
               ...
               ftype == LLAMA_FTYPE_MOSTLY_IQ2_KS) { // TODO: Possibly spread this tree of FTYPES into its respective tensors categories.
        if (name.find("attn_v.weight") != std::string::npos) {
            if      (qs.model.hparams.n_gqa() >= 4 || qs.model.hparams.n_expert >= 2) new_type = GGML_TYPE_IQ4_K;
            else if (qs.model.hparams.n_gqa() >= 2) new_type = GGML_TYPE_IQ3_K;
            else new_type = ftype == LLAMA_FTYPE_MOSTLY_IQ2_S || ftype == LLAMA_FTYPE_MOSTLY_IQ2_M ? GGML_TYPE_IQ3_S : GGML_TYPE_Q2_K;
            ++qs.i_attention_wv;
        }
        else if (qs.model.hparams.n_expert >= 2 && name.find("attn_k.weight") != std::string::npos) {
            new_type = GGML_TYPE_IQ4_KS;
        }
        else if (qs.model.hparams.n_expert >= 2 && name.find("attn_q.weight") != std::string::npos) {
            ...
            if (qs.i_ffn_down < qs.n_ffn_down/4 || qs.i_ffn_down >= 7*qs.n_ffn_down/8) {
                new_type = ftype == LLAMA_FTYPE_MOSTLY_IQ2_S || ftype == LLAMA_FTYPE_MOSTLY_IQ2_M ? GGML_TYPE_IQ3_XXS : GGML_TYPE_IQ2_S;
            }
            ++qs.i_ffn_down;
        }
        ...
        else if (qs.model.hparams.n_expert >= 2 || new_type != GGML_TYPE_IQ6_K) {
            new_type = GGML_TYPE_Q6_K;
        }
        ...
        else if (qs.model.hparams.n_gqa() >= 4) { // TODO: Possibly reintegrate this into FTYPE conditionality
        ...
        if (qs.params->attn_k_type < GGML_TYPE_COUNT) new_type = qs.params->attn_k_type;
            ...
            if (qs.model.hparams.n_gqa() >= 2) new_type =  GGML_TYPE_Q3_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_XS) {
            if (qs.model.hparams.n_gqa() <= 2) new_type = GGML_TYPE_IQ3_XXS;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_XXS) {
        ...
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_XS && qs.model.hparams.n_expert <= 2) {
            new_type = GGML_TYPE_IQ3_XXS;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_XXS && qs.model.hparams.n_expert <= 2) {
        ...
        if (qs.params->ffn_down_type < GGML_TYPE_COUNT) new_type = qs.params->ffn_down_type;
            ...
            if (i_layer < n_layer/8 || i_layer >= 7*n_layer/8) new_type = GGML_TYPE_Q3_K;
        }
