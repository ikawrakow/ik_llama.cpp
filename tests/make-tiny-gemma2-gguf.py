...
layers (build_gemma2.cpp: il % 2 == 0); hparams.swa_layers is populated to
match (src/llama-hparams.cpp, LLM_ARCH_GEMMA2 case). Deterministic given --seed.
"""
import argparse
import os
import sys
...
N_LAYER   = 26  # build_gemma2.cpp's Q-scaling switch keys off model.type, which the
                # hparams loader only maps for n_layer in {26, 42, 46} (2B/9B/27B); any
                # other count hits its unhandled-default GGML_ABORT. Keep 26 (2B) so the
                # tiny model actually exercises real gemma2 model code, not a size class
...
def main():
    ...
    w = gguf.GGUFWriter(args.out, "gemma2")
    ...
    # gemma2-specific keys (see src/llama-hparams.cpp LLM_ARCH_GEMMA2)
    ...
    # GEMMA2 ties output to token_embd: no separate output.weight tensor.
    for i in range(N_LAYER):
        ...
        wt(f"blk.{i}.post_attention_norm.weight", (N_EMBD,), norm=True)
        ...
        wt(f"blk.{i}.post_ffw_norm.weight", (N_EMBD,), norm=True)
