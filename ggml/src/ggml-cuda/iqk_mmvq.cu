...
#define VDR_IQ4_KNN_Q8_1_MMVQ 4
#define VDR_IQ4_KNN_Q8_1_MMQ  4

__device__ __forceinline__ float vec_dot_iq4_knn_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    const float * dptr = (const float *)vbq;
    const float d = *dptr;
    const uint8_t * values = (const uint8_t *)(dptr + 1);
    const block_iq4_knn * bq4 = (const block_iq4_knn *)(values + 16) + kbx;
    ...
    int sumi = 0;
    for (int j = 0; j < 4; ++j) {
        get_int_from_table_16_shift(q4[j], 0, values, v1, v2);
        sumi = ggml_cuda_dp4a(v1, q8[j+0], sumi);
        sumi = ggml_cuda_dp4a(v2, q8[j+4], sumi);
    }
    return d * __low2float(bq8_1[ib32].ds) * sumi;
}
...
void mul_mat_vec_iq4_knn_q8_1_cuda(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int ncols_y, const int nrows_dst, cudaStream_t stream) {

    iqk_mul_mat_vec_q_cuda<GGML_TYPE_IQ4_KNN, VDR_IQ4_KNN_Q8_1_MMVQ, vec_dot_iq4_knn_q8_1>(vx, vy, dst, ncols_x, nrows_x, nrows_y, ncols_y, nrows_dst, stream);
}
