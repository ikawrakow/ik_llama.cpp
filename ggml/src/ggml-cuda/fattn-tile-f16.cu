...
__launch_bounds__(nwarps*WARP_SIZE, 1)
#endif // !(defined(GGML_USE_HIPBLAS) && defined(__HIP_PLATFORM_AMD__))
static __global__ void flash_attn_tile_ext_f16_fabsum(
        const char * __restrict__ Q,
        const char * __restrict__ K,
        const char * __restrict__ V,
        const char * __restrict__ mask,
    ...
    static_assert(FABSUM_PV_BLOCK % 2 == 0 && FATTN_KQ_STRIDE_TILE_F16 % FABSUM_PV_BLOCK == 0,
                  "FABSUM_PV_BLOCK must be even and divide FATTN_KQ_STRIDE_TILE_F16.");
    static_assert((D/2) % FABSUM_QK_BLOCK == 0, "FABSUM_QK_BLOCK must divide the K.Q chain length D/2.");
...
#pragma unroll
            for (int j_KQ_0 = 0; j_KQ_0 < ncols; j_KQ_0 += nwarps) {
                ...
                float sum = KQ_acc[i_KQ_0/WARP_SIZE][j_KQ_0/nwarps];
                if (use_softcap) {
...
#pragma unroll
            for (int i0 = 0; i0 < FATTN_KQ_STRIDE_TILE_F16; i0 += WARP_SIZE) {
                ...
                kqsum_add += val;
                // The probabilities stay half in shared memory to keep occupancy at 2 blocks/SM on P100.
...
#pragma unroll
    ...
    // Attention sinks (gpt-oss): zero-value logit; only ip==0 adds it so it is counted once across
    // the parallel_blocks KV split. Ported from fattn-vec-f16.cuh.
    if (sinksf && ip == 0) {
...
template <int cols_per_block, int parallel_blocks, bool use_softcap>
void launch_fattn_tile_f16_fabsum_64_128(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * Q = dst->src[0];
    switch (Q->ne[0]) {
        case  64: {
            constexpr int      D = 64;
            constexpr int nwarps = 8;
            fattn_kernel_t fattn_kernel = flash_attn_tile_ext_f16_fabsum<D, cols_per_block, nwarps, parallel_blocks, use_softcap>;
            launch_fattn<D, D, parallel_blocks>(ctx, dst, fattn_kernel, nwarps, cols_per_block, true, true);
        } break;
        case 128: {
            constexpr int      D = 128;
...
void ggml_cuda_flash_attn_ext_tile_f16_fabsum(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (Q->ne[1] <= 16) {
        ...
        constexpr int parallel_blocks = 4;
        if (softcap == 0.0f) {
            launch_fattn_tile_f16_fabsum_64_128<cols_per_block, parallel_blocks, false>(ctx, dst);
        } else {
            launch_fattn_tile_f16_fabsum_64_128<cols_per_block, parallel_blocks, true>(ctx, dst);
        }
        return;
    }
