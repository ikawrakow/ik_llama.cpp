...
// DSV4 concatenates plain F16/BF16/F32 state tensors along dimensions 1 and 2
// and may present non-contiguous views. The previous path only handled
// F32 reliably, so keep this backend operation generic for block-size-one
// element types while rejecting quantized block layouts.
...
static void concat_cont_cuda(
    ...
    if (dim == 0) {
        concat_cont<T, 0><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
                x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
    } else if (dim == 1) {
        concat_cont<T, 1><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
                x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
    } else {
        concat_cont<T, 2><<<num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
                x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
    }
}
...
static __global__ void __launch_bounds__(CUDA_CONCAT_BLOCK_SIZE) concat_non_cont(
        ...
        int64_t ne0, int64_t, int64_t, int64_t,
...
static void concat_cuda(
    ...
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        ...
        if (dim != 3) {
            ...
            CUDA_CHECK(cudaMemcpyAsync((char *) dst->data, src0->data, size0,
                    cudaMemcpyDeviceToDevice, stream));
            CUDA_CHECK(cudaMemcpyAsync((char *) dst->data + size0, src1->data, size1,
                    cudaMemcpyDeviceToDevice, stream));
        }
    } else {
        dim3 grid_dim(dst->ne[1], dst->ne[2], dst->ne[3]);
        auto launch_kernel = [&](auto dim_tag) {
            constexpr int dim_value = decltype(dim_tag)::value;
...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    switch (ggml_type_size(src0->type)) {
        ...
        case 4: concat_cuda<uint32_t>(src0, src1, dst, dim, ctx.stream()); break;
        case 8: concat_cuda<uint64_t>(src0, src1, dst, dim, ctx.stream()); break;
        default: GGML_ABORT("Unsupported type size: %zu", ggml_type_size(src0->type)); break;
    }
}
