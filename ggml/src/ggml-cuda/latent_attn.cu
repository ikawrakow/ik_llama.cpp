...
static void ggml_cuda_op_latent_attn_indexed(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    GGML_ASSERT(!mask || (mask->type == GGML_TYPE_F32 && mask->nb[0] == sizeof(float)));
    GGML_ASSERT(cache->type != GGML_TYPE_Q8_0 ||
                cache->nb[1] == ggml_row_size(cache->type, Dk));
...
void ggml_cuda_op_latent_attn(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    GGML_ASSERT(!mask || mask->nb[0] == sizeof(float)); // rows read contiguously
    ...
    if (cache->type == GGML_TYPE_F32) {
        ...
        GGML_ASSERT(cache->nb[1] == ggml_row_size(cache->type, Dk)); // contiguous rows
