...
| `--fused-indexer-topk, -fidx` | Force the fused indexer topk op | GLM DSA: disabled; OpenPangu: automatic on supporting backends | Use a dedicated op for computing the DSA indexer top_k KV cache entries. OpenPangu selects it automatically only when the scheduled backend supports the exact node. GLM DSA remains explicit opt-in. Bring-up switch (compile-time): build with `-DGGML_OPENPANGU_LEGACY_OPS=1` to force the legacy chain for every OpenPangu fused/specialized op (indexer top-k, latent attention, fused RMSNorm+add, offset RoPE, batched mix, pack-cache-rows) at once; the default build (undefined or 0) selects the capability-gated fused paths. The effective policy is reported in the model load log (`openpangu_ops`). [PR 2098](https://github.com/ikawrakow/ik_llama.cpp/pull/2098) |
